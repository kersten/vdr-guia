...
(function( $ ) {
	$.fn.lionbarsRelative = function(color, showOnMouseOver, visibleBar, visibleBg) {
		var targets = $(this);
		var counter = 0;
				...
				if (!timeout && !visibleBar) {
					...
					setTimeout(function() {
						...
						$('.nbar').stop().animate({ "opacity" : 0 }, 400);
                                ...
                                targets.trigger('scroll', {offsetV: offsetV, offsetH: offsetH});
			}
		});
		...
		function dragScrollbar(targetScrollbar, nWrapTarget, e, factorV, factorH, initScrollX, initScrollY, initX, initY, mouseX, mouseY) {
			...
			if (targetScrollbar.hasClass('vertical')) {
				...
				nWrapTarget.scrollLeft(realOffsetX / factorH);
                        ...
                        targets.trigger('scroll', {offsetV: offsetY, offsetH: offsetX, scrollbarHeight: targetScrollbar.siblings('.scrollbar_bg.vertical').height() + targetScrollbar.height()+2});
		}
